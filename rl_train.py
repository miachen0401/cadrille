...
Observability (W&B)
-------------------
  train/loss            PPO clip loss (last of batch_updates)
  train/mean_reward     mean of G rewards
  train/reward_std      reward diversity (low → collapse)
  train/entropy         mean per-token entropy (should decrease)
  train/clip_fraction   fraction of ratios clipped
  train/ratio_mean/std  ratio diagnostics
  val/mean_reward       greedy reward on held-out val set (should rise)
  val/mean_iou          mean IoU on valid completions
  val/valid_fraction    fraction of valid completions

Usage
-----
# GRPO / Dr. CPPO using YAML config
python rl_train.py --config configs/rl_default.yaml --run-name cadrille-rl-v1

# Override specific values
python rl_train.py --config configs/rl_default.yaml --run-name test --max-steps 100

# Legacy: direct CLI flags (no config file)
...
from reward import compute_reward, compute_rewards_parallel
...
class RLDataset:
    """Loads hard-mined examples from mine_hard_examples.py output pkl."""
...
def run_validation(model, val_examples: list, processor, args) -> dict:
    """Greedy decode one completion per val example; return reward metrics."""
    ...
    for item in val_examples:
        ...
        rewards.append(compute_reward(code, item['gt_mesh_path']))

    valid = [r for r in rewards if r > -10.0]
    return {
        'val/mean_reward':    float(np.mean(rewards)),
        'val/mean_iou':       float(np.mean([r / 10 for r in valid])) if valid else 0.0,
        'val/valid_fraction': float(len(valid) / len(rewards)) if rewards else 0.0,
    }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default=None,
                        help='Path to YAML config (e.g. configs/rl_default.yaml)')
    ...
    parser.add_argument('--hard-examples-pkl', type=str, default=None,
                        help='Hard-mined examples pkl from mine_hard_examples.py (legacy)')
