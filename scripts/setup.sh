#!/usr/bin/env bash
# scripts/setup.sh — install deps and (optionally) download eval data for SFT
#
# Usage:
#   bash scripts/setup.sh            # install Python deps only
#   bash scripts/setup.sh --data     # deps + download SFT checkpoint + test meshes from HF
#
set -euo pipefail
...
echo ""
echo "Setup complete."
echo "  SFT   : bash scripts/run_sft.sh --config configs/sft/mix_1_2_2.yaml"
echo "  Eval  : bash scripts/run_eval.sh --checkpoint <path> --split deepcad_test_mesh --mode pc_img"
